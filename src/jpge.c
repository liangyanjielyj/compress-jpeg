...
static uint8 m_num_components;
static uint8 m_comp_h_samp[3], m_comp_v_samp[3];
static int m_image_x, m_image_y, m_image_bpp, m_image_bpl;
static int m_image_x_mcu, m_image_y_mcu;
static int m_image_bpl_xlt, m_image_bpl_mcu;
static int m_mcus_per_row;
static int m_mcu_x, m_mcu_y;
static uint8 *m_mcu_lines[16];
static uint8 m_mcu_y_ofs;
static sample_array_t m_sample_array[64];
static int16 m_coefficient_array[64];
...
static int m_last_dc_val[3];
enum { JPGE_OUT_BUF_SIZE = 2048 };
static uint8 m_out_buf[JPGE_OUT_BUF_SIZE];
static uint8 *m_pOut_buf;
static uint m_out_buf_left;
static uint32 m_bit_buffer;
static uint m_bits_in;
static bool m_all_stream_writes_succeeded;
static uint8 *m_pBuf;
static uint m_buf_size, m_buf_ofs;
...
bool jpg_open(int p_x_res, int p_y_res, int src_channels, const params &comp_params)
{
  ...
  compute_quant_table(m_quantization_tables[0], s_std_lum_quant, comp_params);
  compute_quant_table(m_quantization_tables[1], m_params.m_no_chroma_discrim_flag ? s_std_lum_quant : s_std_croma_quant, comp_params);
